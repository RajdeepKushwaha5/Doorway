...
```bash
npm install && npm test      # 674 tests, no network, no credentials
...
**674 tests, no network and no credentials required.** Verified by deleting `.env` and running the suite.
...
cd Doorway
npm install
npm run check                # typecheck, lint, then 674 tests. All offline
```
...
AI coding tools assisted the implementation. Every architecture decision, platform finding and trade-off recorded here was reviewed and is explainable by the author, and the 674 tests are the check on all of it.
