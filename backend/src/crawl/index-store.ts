...
/**
 * Load a shipped index when the live one is empty.
 *
 * The deployment target has no persistent disk, so every restart begins with an
 * empty index and the first student of the day gets the product at its worst.
 * Crawling on boot would fix that and cost hundreds of paid requests every time
 * the instance wakes, which on a free tier is often.
 *
 * So the index travels with the code, the same way the collector fleet does.
 * The file is real crawl output rather than invented data: every record in it
 * was fetched from a live page through Bright Data and passed the same checks
 * as anything found at runtime. Committing it is shipping a cache, not seeding
 * a demo.
 *
 * Only ever loaded into an empty index, so it can never overwrite what a real
 * crawl has since found.
 */
export async function seedIndex(
  index: OpportunityIndex,
  path: string | undefined,
): Promise<{ seeded: number; reason: 'seeded' | 'already-populated' | 'no-seed-file' }> {
  if (path === undefined || path.trim() === '') return { seeded: 0, reason: 'no-seed-file' };

  const existing = await index.all();
  if (existing.length > 0) return { seeded: 0, reason: 'already-populated' };

  try {
    const raw = await readFile(path, 'utf8');
    const parsed: unknown = JSON.parse(raw);
    if (!Array.isArray(parsed)) return { seeded: 0, reason: 'no-seed-file' };

    const drafts = (parsed as OpportunityDraft[]).filter(
      (entry) => typeof entry?.sourceUrl === 'string' && typeof entry.title === 'string',
    );
    const merged = await index.merge(drafts);
    return { seeded: merged.added, reason: 'seeded' };
  } catch {
    // A missing or malformed seed must not stop the server starting. An API
    // that refuses to boot over a cache would be a strange thing to build.
